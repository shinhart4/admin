  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onCreate} className="px-4 py-2 rounded bg-blue-600 text-white">+ Nouvelle session</button>
      </div>
      {loading ? <Loader/> : <Table<Session> columns={columns} data={data} onEdit={onEdit} onDelete={onDelete} />}
      ...
      <ConfirmDialog open={confirm.open} message={`Supprimer la session #${confirm.row?.id} ?`} onCancel={()=>setConfirm({open:false,row:null})} onConfirm={handleConfirmDelete} />
    </div>
  );
};
